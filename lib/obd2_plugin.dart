...
class Obd2Plugin {
  ...
  Future<int> configObdWithJSON(String stringJson, {int lastIndex = 0, int requestCode = 2}) async {
    ...
    if (!configed) {
      Future.delayed(
          Duration(
              milliseconds:
              stm[lastIndex]["command"] == "AT Z" || stm[lastIndex]["command"] == "ATZ"
                  ? 1000
                  : 100), () {
  ...
  Future<void> setOnDataReceived(
      Function(String command, String response, int requestCode) onResponse,
      Function(List<String> dtcCodes) onResult,
      ) async {
    String response = "";
        ...
        if (!string.contains('>')) {
          ...
          if (this.onResponse != null) {
            if (commandMode == Mode.parameter) {
              dynamic dyResponse = "";
              String type =
              runningService["description"].toString().replaceAll("<", "").replaceAll(">", "");
              ...
              if (validResponse.contains(runningService["unit"].toString().toUpperCase()) ||
                ...
                validResponse =
                    validResponse.replaceAll(runningService["unit"].toString().toUpperCase(), "");
                validResponse =
                    validResponse.replaceAll(runningService["unit"].toString().toLowerCase(), "");
              }
              if (runningService["description"].toString().contains(", ")) {
                List<String> bytes =
                _calculateParameterFrames(runningService["PID"], validResponse.toString());
                ...
                try {
                  for (int i = 0; i < bytes.length; i++) {
                    formula = formula.replaceAll(
                        "[${i.toString()}]", int.parse(bytes[i], radix: 16).toRadixString(10));
                  }
                  Parser p = Parser();
